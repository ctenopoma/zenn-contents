...
**禁止されたはずの列をもう一度作って持ってきます**。
しかも追加されるのは中身が同じ「別の変数」なので、$x_p \le 0$ という枝は最初から無かったことになります。
追加 → また禁止 → また生成、と繰り返して、探索がいつまでも進みません。

:::message alert
これは机上の話ではありません。前回の記事で SCIP の Pricer プラグインを使い、
マスター変数を整数にして分枝させたところ、価格付け問題は同じパターンを何千回も返し、
そのうち79回が「別の変数」としてモデルに追加されました。
中身が同じ列が79本あるので、そのうち1本に上限をかけても意味がありません。
:::
